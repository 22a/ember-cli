...
module.exports = class Watcher extends CoreObject {
  constructor(_options) {
    ...
    if (isExperimentEnabled('BROCCOLI_WATCHER')) {
      ...
      this.watcher.on('buildFailure', this.onBuildFailure.bind(this));
    } else {
  ...
  _setupBroccoliWatcherBuild() {
    try {
      fs.writeFile('lastBroccoliState.json', JSON.stringify({name: 'BuildStart'}), () => {});
    } catch (_e) {}
  ...
  onBuildFailure(error) {
    try {
      fs.writeFile('lastBroccoliState.json', JSON.stringify(error), () => {});
    } catch (_e) {}
  }

  didChange(results) {
    try {
      fs.writeFile('lastBroccoliState.json', JSON.stringify({name: 'BuildSuccess'}), () => {});
    } catch (_e) {}
